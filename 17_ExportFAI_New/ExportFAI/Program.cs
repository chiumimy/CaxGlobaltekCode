...
public class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        try
        {
            ...
            {
                //取得本機IPQC.xls路徑
                DefineVariable.FAIPath = string.Format(@"{0}\{1}", "D:", "FAI.xls");
            }
            ...
            string FAIFolderPath = "";
            #region 建立FAIFolder資料夾

            FAIFolderPath = string.Format(@"{0}\{1}_FAI", Path.GetDirectoryName(displayPart.FullPath), PartNo);
            if (!Directory.Exists(FAIFolderPath))
            {
                System.IO.Directory.CreateDirectory(FAIFolderPath);
            }

            #endregion

            //設定輸出路徑
            string[] FolderFile = System.IO.Directory.GetFileSystemEntries(FAIFolderPath, "*.xls");
            string OutputPath = string.Format(@"{0}\{1}_{2}_{3}", FAIFolderPath, PartNo, "FAI", (FolderFile.Length + 1) + ".xls");
                ...
                if (dataOK)
                {
                    ...
                    for (int i = 0; i < SheetCount; i++)
                    {
                        ...
                        foreach (DisplayableObject singleObj in SheetObj)
                        {
                            ...
                            //判斷是否為FAI屬性
                            if (singleObjExcel == "" || singleObjExcel != "FAI") continue;
                    ...
                    workBook = excelApp.Workbooks.Open(DefineVariable.FAIPath);
                    ...
                    workRange[10, 2] = PartDescription;
                    #endregion

                    //取得第二頁sheet
                    workSheet = (Excel.Worksheet)workBook.Sheets[2];
                    #region 處理第二頁
                    workRange = (Excel.Range)workSheet.Cells;
                    workRange[10, 1] = PartNo;
                    workRange[10, 2] = PartDescription;
                    #endregion
                    ...
                    workRange = (Excel.Range)workSheet.Cells;
                    workRange[10, 1] = PartNo;
                    workRange[10, 5] = PartDescription;
