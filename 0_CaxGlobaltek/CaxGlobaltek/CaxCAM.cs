    ...
    public class CaxCAM
    {
        ...
        public static bool CreateMillToolNxopen(int toolNo, MillingToolData millToolData, out Tool millTool)
        {
            ...
            try
            {
                ...
                //頸柄銜接階段
                if (double.Parse(millToolData.ALPHA_ANGLE) != 90.0 && double.Parse(millToolData.SHANK_DIAMETER) != connectDiameter) //90度或是直徑一樣代表沒有銜接階段
                {
                    //設定直徑
        ...
        //建立鑽頭
        public static bool CreateDrillToolNxopen(int toolNo, MillingToolData millToolData, out Tool millTool)
        {
            millTool = null;
            try
            {
                ...
                //建立刀具
                string toolType = "drill";
                string toolSubtype = "DRILLING_TOOL";
                string toolName = "";
                string preToolName = string.Format("T{0}_{1}", toolNo, millToolData.CUTTER_NAME);
                GetToolName(preToolName, "", out toolName);
                ...
                //設定刀具參數
                NXOpen.CAM.DrillStdToolBuilder drillBuilder;
                drillBuilder = workPart.CAMSetup.CAMGroupCollection.CreateDrillStdToolBuilder(createTool);

                //設定刀具直徑(Tool>Dimensions>Diameter)
                drillBuilder.TlDiameterBuilder.Value = double.Parse(millToolData.DIAMETER);
                //設定錐度(Tool>Dimensions>Point Angle)
                drillBuilder.TlPointAngBuilder.Value = double.Parse(millToolData.FLUTE_A_ANGLE);
                //設定刀尖R角(Tool>Dimensions>Corner Radius)
                drillBuilder.TlCor1RadBuilder.Value = double.Parse(millToolData.RADIUS);
                //設定刃長(Tool>Dimensions>Flute Length)
                drillBuilder.TlFluteLnBuilder.Value = double.Parse(millToolData.FLUTE_LENGTH);
                //設定刃數(Tool>Dimensions>Flutes)
                drillBuilder.TlNumFlutesBuilder.Value = int.Parse(millToolData.FLUTE_NUM);
                ...
                if (double.Parse(millToolData.NECK_LENGTH) != 0.0)
                {
                    ...
                    {
                        //刀長=NL
                        drillBuilder.TlHeightBuilder.Value = double.Parse(millToolData.NECK_LENGTH);
                        //記錄刀長
                        totalLength = double.Parse(millToolData.NECK_LENGTH);
                        ...
                        else
                        {
                            ...
                            //刃側斜角(弧度)
                            double fluteAngleB = CommonFunction.DegreeToRadian(double.Parse(millToolData.FLUTE_B_ANGLE));
                            double toolLength = drillBuilder.TlHeightBuilder.Value;
                            connectDiameter = double.Parse(millToolData.DIAMETER) + toolLength * Math.Tan(fluteAngleB) * 2;
                        }
                    }
                    else //刀頭型
                    {
                        //刀長=刃長
                        drillBuilder.TlHeightBuilder.Value = double.Parse(millToolData.FLUTE_LENGTH);
                        //記錄刀長
                        totalLength = double.Parse(millToolData.FLUTE_LENGTH);
                        ...
                        if (double.Parse(millToolData.BETA_ANGLE) != 90.0) //90度代表沒有銜接階段
                        {
                            ...
                            sectionRadius = 0.0;
                            //新增階段
                            drillBuilder.HolderSectionBuilder.Add(sectionIndex, sectionDiameter, sectionLength, sectionTaper, sectionRadius);
                            sectionIndex++;
                            //總長修改
                            totalLength += sectionLength;
                        }
                ...
                millTool = (Tool)drillBuilder.Commit();
                drillBuilder.Destroy();

            }
            catch (System.Exception ex)
            {
                CaxLog.ShowListingWindow("CreateDrillToolNxopen()..." + ex.Message);
                return false;
            }
            return true;
        }
