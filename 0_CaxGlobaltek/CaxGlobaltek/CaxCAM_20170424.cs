...
namespace CaxGlobaltek
{
    public class CaxCAM
    {
        ...
        public static bool CreateMillToolNxopen(int toolNo, MillingToolData millToolData, out Tool millTool)
        {
            ...
            try
            {
                ...
                if (double.Parse(millToolData.NECK_LENGTH) != 0.0)
                {
                    ...
                    else //刀頭型
                    {
                        ...
                        if (double.Parse(millToolData.BETA_ANGLE) != 90.0) //90度代表沒有銜接階段
                        {
                            ...
                            //設定長度
                            sectionLength = GetCornHeight(double.Parse(millToolData.NECK_DIAMETER), sectionDiameter, double.Parse(millToolData.BETA_ANGLE));
                            //設定錐度
                ...
                if (double.Parse(millToolData.ALPHA_ANGLE) != 90.0) //90度代表沒有銜接階段
                {
                    ...
                    //設定長度
                    sectionLength = GetCornHeight(double.Parse(millToolData.SHANK_DIAMETER), connectDiameter, double.Parse(millToolData.ALPHA_ANGLE));
                    //設定錐度
        ...
        public static double GetCornHeight(double upperD, double lowerD, double taperAngle)
        {
            double height = 0.0;

            double diff = (upperD - lowerD) / 2;

            double tangentValue = Math.Tan(Math.PI * taperAngle / 180.0);

            height = diff / tangentValue;
