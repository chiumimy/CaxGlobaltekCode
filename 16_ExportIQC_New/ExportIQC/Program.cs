...
public class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        try
        {
            ...
            {
                //取得本機IPQC.xls路徑
                DefineVariable.IQCPath = string.Format(@"{0}\{1}", "D:", "IQC.xls");
            }
            ...
            string IQCFolderPath = "";
            #region 建立IQCFolder資料夾
            IQCFolderPath = string.Format(@"{0}\{1}_IQC", Path.GetDirectoryName(displayPart.FullPath), PartNo);
            if (!Directory.Exists(IQCFolderPath))
            {
                System.IO.Directory.CreateDirectory(IQCFolderPath);
            }

            #endregion

            //設定輸出路徑
            string[] FolderFile = System.IO.Directory.GetFileSystemEntries(IQCFolderPath, "*.xls");
            string OutputPath = string.Format(@"{0}\{1}_{2}_{3}", IQCFolderPath, PartNo, "IPQC", (FolderFile.Length + 1) + ".xls");
                ...
                if (dataOK)
                {
                    ...
                    for (int i = 0; i < SheetCount; i++)
                    {
                        ...
                        foreach (DisplayableObject singleObj in SheetObj)
                        {
                            ...
                            //判斷是否為IPQC屬性
                            if (singleObjExcel == "" || singleObjExcel != "IQC") continue;
                    ...
                    workBook = excelApp.Workbooks.Open(DefineVariable.IQCPath);
                    workSheet = (Excel.Worksheet)workBook.Sheets[1];

                    //由檢驗總數開啟符合總數的頁數
                    status = CaxExcel.AddNewSheet(DicDimensionData.Keys.Count, 12, excelApp, workSheet);
                    if (!status)
                    {
                    ...
                    //更改每一個Sheet的名稱與頁數
                    CaxExcel.ModifySheet(PartNo, "IQC", workBook, workSheet, workRange);
                    if (!status)
                    {
                        workBook.Close(Type.Missing, Type.Missing, Type.Missing);
                        excelApp.Quit();
                    ...
                    #region 新版本
                    CaxExcel.IQCRowColumn sIQCRowColumn = new CaxExcel.IQCRowColumn();
                    int currentSheet_Value, count = 0;
                    for (int i = 0; i < DicDimensionData.Keys.Count; i++)
                    {
                        CaxExcel.GetIQCRowColumn(i, out sIQCRowColumn);
                        ...
                        status = CaxExcel.MappingDimenData(cCom_Dimension, workSheet, sIQCRowColumn.DimensionRow, sIQCRowColumn.DimensionColumn);
                        if (!status)
                        {
                        ...
                        workRange[sIQCRowColumn.GaugeRow, sIQCRowColumn.GaugeColumn] = cCom_Dimension.instrument;
                        workRange[sIQCRowColumn.BallonRow, sIQCRowColumn.BallonColumn] = cCom_Dimension.ballon;
                        workRange[sIQCRowColumn.LocationRow, sIQCRowColumn.LocationColumn] = cCom_Dimension.location;
                        workRange[sIQCRowColumn.PartNoRow, sIQCRowColumn.PartNoColumn] = PartNo;
                        workRange[sIQCRowColumn.MaterialRow, sIQCRowColumn.MaterialColumn] = material;
                        #endregion
                    }
                    #endregion
                    ...
                    for (int i = 0; i < listDimensionData.Count; i++)
                    {
                        ...
                        status = CaxExcel.MappingDimenData(listDimensionData[i], workSheet, sIQCRowColumn.DimensionRow, sIQCRowColumn.DimensionColumn);
                        if (!status)
                        {
                            MessageBox.Show("MappingDimenData時發生錯誤，請聯繫開發工程師");
                            workBook.Close(Type.Missing, Type.Missing, Type.Missing);
                        ...
                        #region 檢具、頻率、Max、Min、泡泡、泡泡位置、料號、日期
                        workRange[sIQCRowColumn.GaugeRow, sIQCRowColumn.GaugeColumn] = listDimensionData[i].instrument;
                        workRange[sIQCRowColumn.BallonRow, sIQCRowColumn.BallonColumn] = listDimensionData[i].ballon;
                        workRange[sIQCRowColumn.LocationRow, sIQCRowColumn.LocationColumn] = listDimensionData[i].location;
                        workRange[sIQCRowColumn.PartNoRow, sIQCRowColumn.PartNoColumn] = PartNo;
                        workRange[sIQCRowColumn.MaterialRow, sIQCRowColumn.MaterialColumn] = material;
                        #endregion
                    }
